...
export async function loadPricing(timeoutMs = 4000): Promise<Map<string, Price>> {
...
export function priceForModel(prices: Map<string, Price>, model: string): Price | undefined {
  const cand: string[] = []
  ...
  if (/claude-\d-\d/.test(base)) cand.push(base.replace(/(claude-)(\d)-(\d)/, '$1$2.$3'))
